...
suite('watcher-test', () => {
    ...
    after(async () => {
        await fileService.delete(baseURI, { recursive: true });
    });

    test('watch file', async () => {
        const fileURI = URI.join(baseURI, 'file.txt');
        await fileService.createFile(fileURI, DataBuffer.alloc(0), { overwrite: true });
        ...
        await fileService.writeFile(fileURI, DataBuffer.fromString('hello world'), { create: false, overwrite: true, });
    ...
    test.skip('watch directory', async () => {
        ...
        await fileService.writeFile(fileURI, DataBuffer.fromString('hello world'), { create: true, overwrite: true, });
