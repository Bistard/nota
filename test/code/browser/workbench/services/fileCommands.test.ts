...
import { IncrementFileType, incrementFileName } from 'src/workbench/services/fileTree/fileCommands';

suite('incrementFileName-test', () => {

	test('Increment file name without any version', function () {
		const name = 'test.js';
		const result = incrementFileName(name, false, IncrementFileType.Simple);
	...
	test('Increment folder name without any version', function () {
		const name = 'test';
		const result = incrementFileName(name, true, IncrementFileType.Simple);
		assert.strictEqual(result, 'test copy');
	});
...
suite('Files - Increment file name smart', () => {
	...
	test('Increment file name without any version', function () {
		const name = 'test.js';
		const result = incrementFileName(name, false, IncrementFileType.Smart);
		assert.strictEqual(result, 'test.1.js');
	});

	test('Increment folder name without any version', function () {
		const name = 'test';
		const result = incrementFileName(name, true, IncrementFileType.Smart);
